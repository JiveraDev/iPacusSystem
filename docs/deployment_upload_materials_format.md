...
`MAINTENANCE_REPORT_EMAIL` receives the sanitized diagnostic messages submitted through the maintenance page's **Report problem** button. If it is omitted, the backend uses `MAIL_REPLY_TO`, then `MAIL_FROM_ADDRESS`.
