...
- HEAD reviewed: `7459581` (`update: added Push Notification`, 2026-06-11)
- Commit count on `HEAD`: 51
- Working tree at review time: dirty, with many modified files, deleted older DDL/migration files, and new untracked files.
- Canonical DB file supplied by user: `DDL/database_ddl_20260613_054826.sql`
...
| Consent file management | Implemented, needs workflow QA | `452a044`, later consent usage in current uncommitted work | Consent template CRUD exists; diagnosis/boarding consent usage is newer current work |
...
| Online consultations / Jitsi | Implemented, needs environment setup | `3560ca3`, `docs/jitsi-self-host.md`, online consult PHP/components | Needs Jitsi URL/env and full start/join/end testing |
| Veterinarian workflow and EMR | Implemented, still active | `b79e672`, `bf998a9`, current modified vet/medical files | Diagnosis, approved queue, my list, EMR, histories |
...
| Boarding / pet hotel management | Implemented, still active | `bf998a9`, `54e6f1a`, current modified boarding files | Room management, assignments, observations, tasks, documents, consent current work |
...
| Payment methods management | Current uncommitted feature | Untracked `php/payment_methods.php`, `PaymentMethodsManagement.jsx`, `paymentMethodService.js` | Needs explicit backlog item and QA; not in last commit |
| Record update request workflow | Current uncommitted feature | Untracked `php/record_update_requests.php`, `RecordUpdateRequestsManagement.jsx`, `VetRecordUpdateRequests.jsx` | Needs lifecycle QA and DB alignment |
| Server health / downtime handling | Current uncommitted feature | Untracked `ServerDownPage.jsx`, modified `apiClient.js`, `App.jsx` | Needs API failure testing |
| Diagnosis extra consent and optional vaccination | Current uncommitted feature | Modified `VetDiagnosis.jsx`, `vet_diagnoses.php`, medical record files | Must verify diagnosis can save without vaccination and with extra consent |
| Boarding liability consent before payment/submit | Current uncommitted feature | Modified `PetHotel.jsx`, `add_booking.php`, `get_bookings.php` | Canonical DDL lacks `bookings.consent_forms` and `bookings.consent_status` |
| TV status display/subdomain | Future backlog | User request, no committed implementation found | Best as same project route plus optional subdomain |
...
## Current Working-Tree Backlog Assumptions

These items are not fully represented by committed history because they are modified or untracked in the working tree.

| Item | Evidence | Backlog Assumption | Needed Follow-Up |
| --- | --- | --- | --- |
| Consolidated current DDL | New `DDL/database_ddl_20260613_054202.sql`, `DDL/database_ddl_20260613_054826.sql`; older migration files deleted in working tree | DB source is being consolidated around full DDL exports | Decide whether to keep older migrations for history or replace them with one canonical DDL plus new migrations |
| Boarding consent storage | Modified `php/add_booking.php`, `php/get_bookings.php`, `src/components/PetOwnerDashboard/PetHotel.jsx` | Boarding consent is being added to booking submit and admin fetch flow | Add explicit DB migration for `bookings.consent_forms` and `bookings.consent_status` or export fresh DDL |
| Diagnosis extra consent | Modified `src/components/VetrinarianComponents/VetDiagnosis.jsx`, `php/vet_diagnoses.php` | Vet can add extra consent forms/signatures during diagnosis | QA: save diagnosis with no vaccine; save diagnosis with extra consent; confirm attachments render in history/EMR |
| Medical records print/email | Modified `src/components/PetOwnerDashboard/MedicalRecords.jsx`, `php/pet_medical_records.php`, `src/components/VetrinarianComponents/VetPetsEMR.jsx` | Medical records were expanded and owner actions redesigned | QA: print, email copy, owner visibility, image preview, grouped records |
| Payment method settings | New `php/payment_methods.php`, `PaymentMethodsManagement.jsx`, `paymentMethodService.js`, `usePaymentMethods.js` | Super admin payment method management is in progress | QA OTP, QR image upload, active/inactive methods, payment forms consuming methods |
| Record update requests | New `php/record_update_requests.php`, admin/vet components and service | Record correction lifecycle is in progress | QA owner request, admin review, vet assignment, completion, notification if required |
| API health / outage handling | New `ServerDownPage.jsx`, modified `apiClient.js`, `App.jsx` | App now detects API/database failure | QA with stopped PHP server and stopped DB |
| Notification polish | Modified notification helpers/services/components | Push/email/in-app notification behavior is still being refined | QA unread/read all, push permission, reminder runner, category preferences |
| Upload and preview polishing | Modified `upload.php`, `photo-viewer.jsx`, image helpers | Upload handling and preview behavior are still active | Add PDF/document preview backlog; validate upload type restrictions |
| Route/role expansion | Modified `Dashboard.jsx`, `dashboardRouter.jsx` | New pages are being added to dashboard navigation | Keep debug bypass untouched until separate security task |

## Gantt-Ready Backlog Table

| ID | Backlog Item | Type | Priority Assumption | Depends On | Acceptance Criteria |
| --- | --- | --- | --- | --- | --- |
| BL-001 | Stabilize canonical database schema | Technical | High | Canonical DDL decision | One deployable schema/migration path exists; current code columns are included |
...
| BL-021 | Add TV status display page | Feature | Medium | Queue/booking/diagnosis/payment APIs | Read-only full-screen display auto-refreshes and hides sensitive owner/diagnosis details |
...
| BL-025 | Final regression test pass | QA | High | All feature items | `npm run lint`, `npm run build`, PHP syntax checks, manual flow checklist completed |
...
### Phase 9: Current Uncommitted Work

Evidence:

- New/untracked files include payment methods, record update requests, server-down page, current DDL exports, and the system inventory document.
- Modified files include diagnosis, boarding, medical records, notifications, API client, upload, and profile/auth services.

Assumption:

- Current work is focused on finalizing business rules and operational polish: optional vaccination, extra consents, boarding consent, medical record print/email, payment methods, record update requests, API health, and DB documentation.
...
| DDL/migration confusion | `tables.sql` existed historically, later deleted; old migrations currently deleted in worktree; new canonical DDL exists | Create one clear database migration strategy |
