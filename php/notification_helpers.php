...
function notification_send_online_consultation_event(PDO $pdo, int $onlineConsultationId, string $event): void
{
    ...
    if ($event === 'vet_ready' && $ownerUserId > 0) {
        notification_create_event($pdo, [
            ...
            'category' => 'schedule_reminders',
            'title' => 'Your veterinarian is in the call',
            'message' => "{$vetName} is waiting in {$petName}'s online consultation ({$bookingNumber}). Open this notification to join now.",
            'push_title' => 'Online consultation is live',
            'push_message' => "The veterinarian is waiting for {$petName}. Tap to join the call now.",
            'redirect_path' => "/dashboard/consult/video/{$onlineConsultationId}",
