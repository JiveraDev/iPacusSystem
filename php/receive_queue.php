...
try {
    ...
    $queueStmt = $pdo->prepare("SELECT queue_id, status, service_name FROM queues WHERE queue_id = ? FOR UPDATE");
    ...
    if (in_array(strtolower(trim((string)($queue['service_name'] ?? ''))), ['grooming', 'pet grooming'], true)) {
        ...
        echo json_encode(['error' => 'Grooming queues are handled in Grooming Management, not veterinarian diagnosis.']);
        exit;
    }
