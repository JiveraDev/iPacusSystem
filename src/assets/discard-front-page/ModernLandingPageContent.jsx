...
const services = [
    { id: 'consultation', eyebrow: 'Clinic consultation', title: 'A calmer check-up, from nose to tail.', description: 'Book general check-ups, dental care, laboratory testing, surgery review, and other clinic services around your pet’s needs.', detail: 'In-clinic care', Icon: Stethoscope, media: '/landing-media/vet-consultation.mp4', poster: vetImage, accent: 'mint' },
    { id: 'vaccination', eyebrow: 'Preventive care', title: 'Vaccination records that remember the next step.', description: 'Plan vaccines and parasite-control visits, then keep the date given, veterinarian, and next due date with the pet profile.', detail: 'Vaccines & prevention', Icon: Syringe, media: '/landing-media/pet-vaccination.mp4', poster: vetImage, accent: 'coral' },
    { id: 'grooming', eyebrow: 'Grooming', title: 'Fresh coats, tidy paws, happier pets.', description: 'Request a grooming schedule with pet details and notes so the care team can prepare before confirming the visit.', detail: 'Coat, paw & hygiene care', Icon: Scissors, media: '/landing-media/pet-grooming.mp4', poster: vetImage, accent: 'sun' },
    { id: 'online', eyebrow: 'Online consultation', title: 'Talk to a doctor without leaving your pet’s side.', description: 'For non-emergency concerns, choose a veterinary schedule, complete the PHP 500 consultation flow, and receive notes afterward.', detail: 'Video consult · PHP 500', Icon: Video, image: consultImage, accent: 'blue' },
];
...
function Reveal({ children, className = '', delay = 0 }) {
    ...
    useEffect(() => {
        ...
        if (!('IntersectionObserver' in window)) { element.classList.add('is-visible'); return undefined; }
...
ServiceCard.propTypes = {
    service: PropTypes.shape({ accent: PropTypes.string.isRequired, eyebrow: PropTypes.string.isRequired, title: PropTypes.string.isRequired, Icon: PropTypes.elementType.isRequired }).isRequired,
...
export default function ModernLandingPageContent({ onLogin, onRegister }) {
    ...
    return (
        <div className="landing-modern min-w-0 overflow-x-hidden bg-[#fbfaf6] text-slate-950">
            <header className="landing-header sticky top-0 z-50 border-b border-white/60 bg-[#fbfaf6]/90 backdrop-blur-xl">
                <div className="mx-auto flex min-h-[4.75rem] w-full max-w-[90rem] items-center justify-between gap-4 px-4 sm:px-6 lg:px-10">
                    <a href="#top" className="group flex min-w-0 items-center gap-3" aria-label={`${clinic.name} home`} onClick={(event) => navigate(event, '#top')}>
                        <span className="relative"><span className="absolute -inset-1 rounded-full bg-teal-200/0 blur transition group-hover:bg-teal-200/60" /><img src={logoImage} alt="" className="relative h-11 w-11 object-contain" /></span>
            ...
            <main id="top">
                <section className="landing-hero relative isolate overflow-hidden bg-[#0c3038] text-white">
                    ...
                    <div className="relative mx-auto grid min-h-[calc(100svh-4.75rem)] w-full max-w-[90rem] items-center gap-8 px-4 py-14 sm:px-6 sm:py-16 lg:grid-cols-[minmax(0,0.95fr)_minmax(30rem,1.05fr)] lg:px-10 lg:py-20">
                        <div className="relative z-10 max-w-3xl">
                            <div className="landing-enter landing-enter--1 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3.5 py-2 text-[0.68rem] font-black uppercase tracking-[0.17em] text-teal-100 backdrop-blur-xl"><Sparkles className="h-4 w-4" />Veterinary care in Lucena City</div>
                            ...
                            <div className="landing-enter landing-enter--5 mt-9 flex flex-wrap gap-x-6 gap-y-3 border-t border-white/15 pt-6">{['Pet-specific medical history', 'Non-emergency online consults', 'Two connected Lucena locations'].map((point) => <div key={point} className="flex items-center gap-2 text-xs font-bold text-white/75 sm:text-sm"><span className="flex h-5 w-5 items-center justify-center rounded-full bg-teal-300 text-[#0c3038]"><Check className="h-3 w-3" /></span>{point}</div>)}</div>
                        </div>
                        <div className="landing-enter landing-enter--visual relative z-10 min-h-[24rem] lg:min-h-[39rem]"><Suspense fallback={<div className="landing-pet-stage"><img src="/landing-media/pet-ensemble.png" alt="Dogs, a cat, a parrot, and a rabbit" className="landing-pet-fallback" /></div>}><PetStage /></Suspense><div className="landing-floating-note landing-floating-note--hours"><Clock className="h-4 w-4 text-teal-600" /><span><strong>Open Mon–Sat</strong><small>8:00 AM–6:00 PM</small></span></div><div className="landing-floating-note landing-floating-note--consult"><Video className="h-4 w-4 text-[#e05a47]" /><span><strong>Online vet</strong><small>PHP 500 consultation</small></span></div></div>
                    </div>
                    <div className="relative z-10 mx-auto grid w-full max-w-[90rem] gap-px border-t border-white/10 bg-white/10 sm:grid-cols-3">{[['Monday–Saturday', 'Clinic days'], ['2 Lucena locations', 'Connected care'], ['Dogs, cats, birds +', 'Companion pets']].map(([value, label]) => <div key={label} className="bg-[#0c3038]/80 px-5 py-4 backdrop-blur-xl sm:px-8"><div className="text-sm font-black">{value}</div><div className="mt-1 text-[0.65rem] font-black uppercase tracking-[0.15em] text-teal-200/70">{label}</div></div>)}</div>
                </section>

                <section id="services" className="landing-section bg-[#fbfaf6] px-4 py-20 sm:px-6 sm:py-24 lg:px-10 lg:py-32">
                    <div className="mx-auto w-full max-w-[90rem]">
                        <Reveal className="max-w-3xl"><div className="landing-kicker text-teal-700">Care in motion</div><h2 className="landing-display mt-4">Choose the moment your pet needs.</h2><p className="mt-5 max-w-2xl text-base font-medium leading-7 text-slate-600 sm:text-lg sm:leading-8">Real care deserves more than a list. Explore each service, see the experience, and start with a clear next step.</p></Reveal>
                        <div className="mt-12 grid gap-8 xl:grid-cols-[minmax(20rem,0.72fr)_minmax(0,1.28fr)] xl:items-stretch">
                            ...
                            <Reveal delay={120} className="h-full"><article className={`landing-service-showcase landing-service-showcase--${activeService.accent}`}><div className="landing-service-showcase__media">{activeService.media ? <video key={activeService.media} src={activeService.media} poster={activeService.poster} autoPlay muted loop playsInline preload="metadata" /> : <img src={activeService.image} alt="Doctor providing an online consultation" className="landing-media-kenburns" />}<div className="landing-service-showcase__scrim" /><div className="landing-service-showcase__badge"><span className="landing-service-showcase__badge-icon"><ActiveIcon className="h-5 w-5" /></span>{activeService.detail}</div></div><div className="landing-service-showcase__copy"><div className="landing-kicker text-teal-700">{activeService.eyebrow}</div><h3 className="mt-3 text-2xl font-black leading-tight tracking-[-0.03em] sm:text-3xl">{activeService.title}</h3><p className="mt-4 text-base font-medium leading-7 text-slate-600">{activeService.description}</p><button type="button" onClick={register} className="mt-6 inline-flex items-center gap-2 text-sm font-black text-teal-700 transition hover:gap-3">Start this care request <ArrowRight className="h-4 w-4" /></button></div></article></Reveal>
                        </div>
                    </div>
                </section>
                ...
                <section id="journey" className="landing-section overflow-hidden bg-[#f2f8f5] px-4 py-20 sm:px-6 sm:py-24 lg:px-10 lg:py-32">
                    <div className="mx-auto w-full max-w-[90rem]">
                        <Reveal className="grid gap-6 lg:grid-cols-[1fr_auto] lg:items-end"><div><div className="landing-kicker text-teal-700">A gentler journey</div><h2 className="landing-display mt-4">Care that follows your pet home.</h2></div><button type="button" onClick={register} className="landing-primary-button w-full justify-center px-6 py-3.5 lg:w-auto">Create a pet profile <ArrowRight className="h-4 w-4" /></button></Reveal>
                        <div className="relative mt-12 grid gap-5 lg:grid-cols-3"><div className="landing-journey-line" />{journey.map((step, index) => { const Icon = step.Icon; return <Reveal key={step.number} delay={index * 100} className="relative"><article className="landing-journey-card"><div className="flex items-center justify-between"><span className="landing-journey-card__icon"><Icon className="h-6 w-6" /></span><span className="text-5xl font-black tracking-[-0.08em] text-teal-900/10">{step.number}</span></div><h3 className="mt-8 text-xl font-black">{step.title}</h3><p className="mt-3 text-sm font-medium leading-7 text-slate-600">{step.text}</p></article></Reveal>; })}</div>
                        <Reveal delay={160} className="mt-8"><div className="landing-consult-strip"><div className="landing-consult-strip__image"><img src={consultImage} alt="Online consultation from home" /></div><div className="p-6 sm:p-8 lg:p-10"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-teal-100 text-teal-800"><MessageCircleHeart className="h-6 w-6" /></div><h3 className="mt-6 text-2xl font-black tracking-[-0.03em] text-white sm:text-3xl">Some questions can begin from home.</h3><p className="mt-4 max-w-xl text-sm font-medium leading-7 text-white/70 sm:text-base">Online consultations help with suitable non-emergency concerns while preserving the doctor’s notes in your pet’s connected record.</p><button type="button" onClick={register} className="mt-6 inline-flex items-center gap-2 text-sm font-black text-teal-200">Explore online consultation <ArrowRight className="h-4 w-4" /></button></div></div></Reveal>
                    </div>
                </section>
                ...
                <section id="contact" className="landing-section bg-[#fbfaf6] px-4 py-20 sm:px-6 sm:py-24 lg:px-10 lg:py-32"><div className="mx-auto w-full max-w-[90rem]"><Reveal><div className="landing-kicker text-[#d65343]">Two Lucena locations</div><h2 className="landing-display mt-4">Care is closer than you think.</h2></Reveal><div className="mt-10 grid gap-5 lg:grid-cols-2">{locations.map((location, index) => <Reveal key={location.code} delay={index * 90}><a href={location.mapsUrl} target="_blank" rel="noopener noreferrer" className="landing-location-card"><span className="landing-location-card__pin"><MapPin className="h-6 w-6" /></span><span className="min-w-0 flex-1"><span className="text-[0.65rem] font-black uppercase tracking-[0.16em] text-[#d65343]">{location.type}</span><span className="mt-2 block text-xl font-black">{location.name}</span><span className="mt-3 block text-sm font-medium leading-6 text-slate-600">{location.address}</span><span className="mt-5 inline-flex items-center gap-2 text-sm font-black text-teal-800">Open in Maps <ExternalLink className="h-4 w-4" /></span></span></a></Reveal>)}</div><Reveal delay={120} className="mt-6"><div className="landing-contact-bar"><div className="flex items-start gap-3"><Clock className="mt-0.5 h-5 w-5 text-teal-600" /><span><strong>Clinic hours</strong><small>{clinic.openDays}, {clinic.hours}</small></span></div><div className="flex items-start gap-3"><Phone className="mt-0.5 h-5 w-5 text-teal-600" /><span><strong>Call us</strong><small>{clinic.phone}</small></span></div><div className="flex min-w-0 items-start gap-3"><Mail className="mt-0.5 h-5 w-5 text-teal-600" /><span className="min-w-0"><strong>Email</strong><small className="break-all">{clinic.email}</small></span></div></div></Reveal></div></section>

                <section className="bg-[#0c3038] px-4 py-16 text-white sm:px-6 lg:px-10 lg:py-20"><Reveal className="mx-auto flex w-full max-w-[90rem] flex-col justify-between gap-8 lg:flex-row lg:items-center"><div><div className="landing-kicker text-teal-200">Ready when they are</div><h2 className="mt-4 text-3xl font-black tracking-[-0.04em] sm:text-5xl">Make the next care moment easier.</h2></div><button type="button" onClick={register} className="landing-primary-button landing-primary-button--light w-full justify-center px-7 py-4 text-base lg:w-auto">Book a visit <ArrowRight className="h-5 w-5" /></button></Reveal></section>
            </main>
