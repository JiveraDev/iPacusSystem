...
function SelectContent({ children, className }) {
  ...
  const shouldSearch = allowCustom || searchable === true || (searchable !== false && selectableItemCount > 8);
  ...
  return createPortal(
    <div
      ...
      className={cn(
        "fixed z-[2200] overflow-hidden rounded-lg border border-slate-200 bg-white text-slate-950 shadow-xl animate-in fade-in zoom-in-95 motion-reduce:animate-none dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100",
        className
      )}
    >
      {shouldSearch && (
...
function SelectItem({ value: itemValue, children, className, disabled = false }) {
  ...
  return (
    <div
      ...
      className={cn(
        "relative flex min-h-9 w-full cursor-default select-none items-center rounded-md py-1.5 pl-8 pr-2 text-sm outline-none hover:bg-slate-100 hover:text-slate-900 focus-visible:ring-2 focus-visible:ring-blue-600 data-[disabled]:pointer-events-none data-[disabled]:opacity-50 dark:hover:bg-slate-800 dark:hover:text-slate-100",
        isSelected && "bg-blue-50 font-bold text-blue-800 dark:bg-blue-950/60 dark:text-blue-200",
        className
      )}
    >
